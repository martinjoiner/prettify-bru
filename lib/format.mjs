...
async function formatBlock(fileContents, blockName, config) {
    ...
    // Strip console output for JavaScript blocks
    if (
        config.stripConsoleOutput !== false &&
        ['script:pre-request', 'script:post-response', 'tests'].includes(blockName)
    ) {
        unindented = stripConsoleOutput(unindented, config.stripConsoleOutput)
    }
...
function stripConsoleOutput(blockContents, stripConsoleOutput) {
    ...
    // Build regex pattern for the types we want to strip
    // This regex matches the entire line including the optional newline
    const typesPattern = typesToStrip.join('|')
    const consoleRegex = new RegExp(`^\\s*console\\.(${typesPattern})\\([^)]+\\);*\\s*\\n?`, 'gm')

    return blockContents.replace(consoleRegex, '')
}
