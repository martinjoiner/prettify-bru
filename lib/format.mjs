...
async function formatBlock(fileContents, blockName, config) {
    ...
    // Strip console output for JavaScript blocks after Prettier formatting
    // This ensures we're working with normalized, single-line console statements
    if (
        config.stripConsoleOutput !== false &&
        ['script:pre-request', 'script:post-response', 'tests'].includes(blockName)
    ) {
        reformatted = stripConsoleOutput(reformatted, config.stripConsoleOutput)
    }
...
/**
 * Strip console output statements based on configuration
 *
 * @param {string} blockContents
 * @param {boolean|string[]} stripConsoleOutput Configuration value
 * @returns {string}
 */
function stripConsoleOutput(blockContents, stripConsoleOutput) {
    if (stripConsoleOutput === false) {
        return blockContents
    }

    let typesToStrip = []
    if (stripConsoleOutput === true) {
        typesToStrip = ['log', 'warn', 'error']
    } else if (Array.isArray(stripConsoleOutput)) {
        typesToStrip = stripConsoleOutput
    }

    if (typesToStrip.length === 0) {
        return blockContents
    }

    // Process line by line to handle console statements with nested parentheses
    ...
    for (const line of lines) {
        ...
        // Check if this line starts with console.X( where X is one of our types to strip
        let shouldStrip = false
        for (const type of typesToStrip) {
            if (trimmedLine.startsWith(`console.${type}(`)) {
                shouldStrip = true
                break
            }
        }
        ...
        if (shouldStrip) {
            ...
            // This handles nested function calls like console.log(func(arg))
            let parenCount = 0
            ...
            // Only skip this line if it's a complete console statement
